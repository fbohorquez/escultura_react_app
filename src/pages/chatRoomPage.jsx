...
const ChatRoomPage = () => {
  ...
  // Auto-scroll al final cuando el componente se monta
  useEffect(() => {
    // Usar un timeout para asegurar que el DOM esté renderizado
    const timeoutId = setTimeout(() => {
      scrollToBottom();
    }, 100);
  ...
  const formatMessageTimeLocal = (timestamp) => {
    return formatMessageTime(timestamp);
  };

  const isMyMessageLocal = (message) => {
    return isMyMessage(message, isAdmin, currentUserId);
  };

  const getMessageSenderNameLocal = (message) => {
    return getMessageSenderName(message, t);
  };
  ...
  return (
		<BackgroundLayout title={getChatTitle()} subtitle={getChatSubtitle()}>
						...
						<div className="messages-list">
							{chatMessages.map((message, index) => (
								<div
									key={index}
									className={`message ${
										isMyMessageLocal(message) ? "my-message" : "other-message"
									}`}
								>
									<div className="message-content">
										{!isMyMessageLocal(message) && (
											<div className="message-sender">
												{getMessageSenderNameLocal(message)}
											</div>
										)}
										<div className="message-text">{message.message}</div>
										<div className="message-time">
											{formatMessageTimeLocal(message.date)}
										</div>
									</div>
								</div>
							))}
